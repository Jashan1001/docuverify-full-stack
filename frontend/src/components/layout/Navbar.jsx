  ...
  return (
    <header className="border-b-3 border-ink bg-paper sticky top-0 z-50">
      <div className="bg-ink text-paper font-mono text-xs py-1 px-6 flex items-center justify-between">
        ...
        {user && (
          <span
            className={`tracking-widest px-2 py-0.5 text-[10px] font-medium ${
              isAdmin
            ...
            {isAdmin && (
              <>
                <NavLink
                  ...
                  active={isActive("/admin/institutions")}
...
function NavLink({ to, active, icon, children }) {
  return (
    <Link
      to={to}
      className={`flex items-center gap-2 font-mono text-xs tracking-widest uppercase px-4 py-2 border-3 transition-all duration-150 ${
        active
          ? "bg-ink text-paper border-ink shadow-brutal"
          : "border-transparent text-muted hover:border-ink hover:text-ink hover:bg-surface-1"
      }`}
    >
      {icon}
      {children}
