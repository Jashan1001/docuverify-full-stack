...
const ASSIGNABLE_ROLES = [ROLES.USER, ROLES.VERIFIER, ROLES.INSTITUTION_ADMIN];
    ...
    <div className="animate-fadeUp">
      ...
      <div className="mb-8">
        ...
        <p className="font-body text-muted mt-2 text-sm">
          Assign roles, manage access, enable/disable accounts.
        </p>
      </div>
        ...
        <>
          <div className="border-3 border-ink shadow-brutal overflow-hidden mb-6">
            <div className="grid grid-cols-5 bg-ink text-paper border-b-3 border-ink">
              {["Name / Email", "Role", "Institution", "Status", "Actions"].map(
                (h) => (
                  <div
                    key={h}
            ...
            {users.length === 0 ? (
              ...
              users.map((u, i) => (
                ...
                >
                  ...
                  <div className="px-4 py-3">
                    <span
                      className={`status-badge text-[10px] border-2 ${
                        u.role === ROLES.ADMIN
                          ? "bg-ink text-paper border-ink"
                          : u.role === ROLES.INSTITUTION_ADMIN
                            ? "bg-accent text-paper border-ink"
                            : u.role === ROLES.VERIFIER
                  ...
                  <div className="px-4 py-3 font-mono text-xs text-muted truncate">
                    {u.institutionName || "—"}
                  </div>
                  <div className="px-4 py-3">
                    <span
                      className={`status-badge text-[10px] border-2 ${u.enabled ? "text-success border-success" : "text-danger border-danger"}`}
                    >
          ...
          <div className="bg-paper border-3 border-ink shadow-brutal-lg w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b-3 border-ink">
              ...
              <button
                ...
                className="font-mono text-muted hover:text-ink text-xl"
              >
                ✕
              </button>
            </div>
